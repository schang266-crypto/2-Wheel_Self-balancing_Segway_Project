...
module Segway_tb_check_steering();
    ...
    initial begin
        $display("Begin testing test case: Check_Steering");
	    ...
	    Check_steering(.clk(clk),.ld_cell_lft(ld_cell_lft),.ld_cell_rght(ld_cell_rght),.send_cmd(send_cmd),.rider_lean(rider_lean),.cmd(cmd),.cmd_sent(cmd_sent),.steerPot(steerPot));

        $display("Finished testing test case: Check_Steering!");
        $stop();
