...
module Segway_tb_auth_blk();
    ...
    initial begin
        $display("Begin testing test case: Check_Auth_block");
	    ...
	    Check_auth_block(.clk(clk),.ld_cell_lft(ld_cell_lft),.ld_cell_rght(ld_cell_rght),.send_cmd(send_cmd),.cmd(cmd),.cmd_sent(cmd_sent),.pwr_up(pwr_up));

        $display("Finished testing test case: Check_Auth_block!");
        $stop();
