...
module Segway(clk,RST_n,INERT_SS_n,INERT_MOSI,INERT_SCLK,
  ...
  ////////////////////////////////////////////////////
  auth_blk iAuth(.clk(clk),.rst_n(rst_n),.RX(RX),.rider_off(rider_off),.pwr_up(pwr_up)); // determine powering up or not from user 
  ...
  inert_intf #(fast_sim) iNEMO(.clk(clk),.rst_n(rst_n),.ptch(ptch),.ptch_rt(ptch_rt),
				   ...
				   .INT(INERT_INT)); // create pitch from integrator
  ...
  steer_en #(fast_sim) iSTR(.clk(clk),.rst_n(rst_n),.lft_ld(lft_ld),
                            .rght_ld(rght_ld),.en_steer(en_steer),.rider_off(rider_off)); // assert en_steer when comparing left/right load
 ...
 ////////////////////////////////////////////////////////////
  ...
  A2D_intf iA2D(.clk(clk),.rst_n(rst_n),.nxt(vld),.lft_ld(lft_ld),.rght_ld(rght_ld),
				...
				.MOSI(A2D_MOSI),.MISO(A2D_MISO)); // Get left/right load data
