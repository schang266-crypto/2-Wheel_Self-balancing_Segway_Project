module ADC128S_FC(clk,rst_n,SS_n,SCLK,MISO,MOSI,ld_cell_lft,ld_cell_rght,steerPot,batt);
  ...
  input MOSI;			// serial data in from master, which contains channel info
  ...
  always_ff @(posedge clk, negedge rst_n)
    if (!rst_n)
	  ...
	  channel <= cmd[13:11]; // rd_data for slave SPI from first transaction 
  ...
  assign A2D_data = {4'b0000,value}; // wrt_data for slave SPI
  ...
  always_comb
    begin
      ...
      case (state)
      FIRST : begin // wait for first transaction to complete (get channel info)
        if (rdy_rise) begin
		      update_ch = 1;
          nxt_state = SECOND;
        end
      end
		  SECOND : begin // wait for second transaction to complete (send A2D data to master)
		    if (rdy_rise)
			    nxt_state = FIRST;
		    else
		      nxt_state = SECOND;
