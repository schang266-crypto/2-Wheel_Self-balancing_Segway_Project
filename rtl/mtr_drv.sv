module mtr_drv(clk,rst_n,lft_spd,rght_spd,OVR_I_lft,OVR_I_rght,PWM1_lft,
  ...
  wire OVR_I_rise;
  wire ovr_I_blank; // ignore OVR_I during first 128 clocks of PWM1 or PWM2 = 1
  ...
  always_ff @(posedge clk, negedge rst_n)
    if (!rst_n) begin
	  ...
	  OVR_I_ff1 <= ~ovr_I_blank & (OVR_I_lft | OVR_I_rght); // ignore OVR_I during blanking period (first 128 clocks of PWM1 or PWM2 = 1)
	  OVR_I_ff2 <= OVR_I_ff1;
	  OVR_I_ff3 <= OVR_I_ff2;
	end
